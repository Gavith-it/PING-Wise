...
import { X, Phone, Mail, Edit, Trash2 } from 'lucide-react';
...
export default function PatientDetailsModal({ patient, onClose, onEdit, onDelete }: PatientDetailsModalProps) {
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="p-4 md:p-6">
          <div className="flex items-center justify-between mb-4 md:mb-6">
            <h3 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">Patient Details</h3>
            <button
              onClick={onClose}
          ...
          <div className="text-center mb-4 md:mb-6">
            <div className={`w-12 h-12 md:w-16 md:h-16 ${patient.avatarColor || 'bg-primary'} rounded-full flex items-center justify-center text-white text-base md:text-xl font-medium mx-auto mb-2 md:mb-3 shadow-md`}>
              {patient.initials || 'P'}
            </div>
            <h4 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white">{patient.name}</h4>
            <p className="text-sm md:text-base text-gray-600 dark:text-gray-400">{patient.age} years old</p>
            <span className={`inline-block text-xs md:text-sm px-2 md:px-3 py-0.5 md:py-1 rounded-full ${getStatusColor(patient.status)} mt-1 md:mt-2`}>
          ...
          <div className="bg-gray-50 dark:bg-gray-700/50 rounded-lg md:rounded-xl p-3 md:p-4 mb-3 md:mb-6">
            ...
            <div className="space-y-1.5 md:space-y-2">
              ...
              <div className="flex items-center space-x-2 md:space-x-3">
                ...
                <span className="text-xs md:text-sm text-gray-700 dark:text-gray-300 break-all">{patient.email || 'N/A'}</span>
              </div>
              {patient.address && (
                <div>
                  <label className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400">Address</label>
                  <p className="text-xs md:text-sm text-gray-700 dark:text-gray-300">{patient.address}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-green-50 dark:bg-green-900/20 rounded-lg md:rounded-xl p-3 md:p-4 mb-3 md:mb-6">
            <h5 className="font-semibold text-sm md:text-base text-gray-900 dark:text-white mb-2 md:mb-3">Personal Information</h5>
            <div className="space-y-2 md:space-y-3">
              <div className="grid grid-cols-2 gap-3 md:gap-4">
                <div>
                  <label className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400">Gender</label>
                  <p className="text-xs md:text-sm text-gray-900 dark:text-white capitalize">
                    {patient.gender || 'N/A'}
                  </p>
                </div>
                <div>
                  <label className="text-xs md:text-sm font-medium text-gray-500 dark:text-gray-400">Date of Birth</label>
            ...
            <div className="space-y-2 md:space-y-3">
              ...
              {patient.medicalNotes && (
                <div>
                  ...
                  <p className="text-xs md:text-sm text-gray-900 dark:text-white">{patient.medicalNotes}</p>
                </div>
              )}
            </div>
          </div>
