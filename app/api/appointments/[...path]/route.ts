/**
 * Appointment API Proxy Route - Catch-all
 * Handles all Appointment API endpoints: /api/appointments, /api/appointments/{id}, etc.
 * 
...
async function handleRequest(
  ...
  try {
    // Reconstruct the path (e.g., ['123'] -> '/appointments/123' or [] -> '/appointments')
    const path = params.path && params.path.length > 0 
      ? `/appointments/${params.path.join('/')}`
      : '/appointments';
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        console.log(`[Appointment Proxy] ${method} ${path} - Status: ${response.status}`);
      }

      // Check for error status codes
      if (!response.ok) {
        const errorText = await response.text().catch(() => 'Unknown error');
        console.error(`[Appointment Proxy] Error ${response.status}:`, errorText);
      ...
      try {
        ...
        if (process.env.NODE_ENV === 'development') {
          console.log(`[Appointment Proxy] Response body:`, responseText.substring(0, 200));
        }
        ...
        console.error('[Appointment Proxy] Error parsing response:', parseError);
        data = null;
      }

      // Normalize null to empty array for GET /appointments endpoint
      if (method === 'GET' && path === '/appointments' && (data === null || data === undefined)) {
        if (process.env.NODE_ENV === 'development') {
          console.log('[Appointment Proxy] Normalizing null response to empty array for /appointments');
        }
        data = [];
      }
      ...
      if (fetchError.name === 'AbortError' || fetchError.code === 'UND_ERR_CONNECT_TIMEOUT') {
        console.error(`Appointment API Proxy timeout (${method}):`, fetchError);
        return NextResponse.json(
          { 
            success: false, 
      ...
      if (fetchError.message?.includes('fetch failed') || fetchError.code === 'ECONNREFUSED') {
        console.error(`Appointment API Proxy connection error (${method}):`, fetchError);
        return NextResponse.json(
          { 
            success: false, 
    ...
    console.error(`Appointment API Proxy error (${method}):`, error);
    return NextResponse.json(
      { 
        success: false, 
